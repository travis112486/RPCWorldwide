...
export default function EditCastingPage() {
  ...
  const loadCasting = useCallback(async () => {
    ...
    const existingRoles: RoleInput[] = (rolesRes.data ?? []).map((r) => ({
      id: r.id,
      name: r.name,
      description: r.description ?? '',
      role_type: (r.role_type as RoleType) ?? '',
      union_requirement: (r.union_requirement as UnionStatus) ?? '',
      pay_rate: r.pay_rate ?? '',
      gender_requirement: (r.gender_requirement as string[]) ?? [],
      age_min: r.age_min != null ? String(r.age_min) : '',
      age_max: r.age_max != null ? String(r.age_max) : '',
      ethnicity_requirement: (r.ethnicity_requirement as string[]) ?? [],
      location_requirement: r.location_requirement ?? '',
      is_open: r.is_open ?? true,
      work_date: r.work_date ? (r.work_date as string).slice(0, 10) : '',
      submission_deadline: r.submission_deadline ? (r.submission_deadline as string).slice(0, 16) : '',
    }));
    setRoles(existingRoles.length > 0 ? existingRoles : [emptyRole()]);
